/**
 * Interchange file for CubeControl desktop ↔ mobile.
 * Filename: `*.cubecontrol.json`. Detected by `format`, not by extension
 * (WhatsApp sometimes renames the file).
 *
 * Keep this schema in sync with apps/desktop/electron/library/shareFormat.ts
 */
...
export function looksLikeShareJson(name: string): boolean {
  const lower = name.toLowerCase();
  return lower.endsWith(".cubecontrol.json") || lower.endsWith(".json");
}
